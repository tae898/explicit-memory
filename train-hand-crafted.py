...
class Memory():
    ...
    def __init__(self, memory_type: str, capacity: dict) -> None:
        ...
        logging.debug(
            f"{memory_type} memory object with size {capacity} instantiated!")
    ...
    def find_duplicate_head(self, elem: list) -> int:
        ...
        for idx, mem in enumerate(self.memory):
            if mem[0] == elem[0]:
                logging.info(
                    f"{mem} has the same head as the new obs {elem} !!!")
                return idx
    ...
    def find_similar(self):
        ...
        logging.debug(
            f"looking for episodic entries that can be compressed ...")
        assert self.memory_type == 'episodic'
        semantic_possibles = [[entry.split()[-1] for entry in entry]
        ...
        if len(semantic_possibles) == len(self.memory):
            ...
            max_key = max(semantic_possibles,
                          key=lambda k: len(semantic_possibles[k]))
            indexes = semantic_possibles[max_key]
            semantic_memory = max_key.split('_')

            logging.warning(f"{len(indexes)} episodic memories can be compressed "
...
def train_only_semantic(policy: dict, data: dict, capacity: dict, save_at: str) -> None:
...
def train_both_episodic_and_semantic(policy: dict, data: dict, capacity: dict, save_at: str) -> None:
            ...
            for idx, obs in enumerate(data[split]):
                ...
                if question in M_e.memory:
                    ...
                    if question in M_s.memory:
                        rewards += 1
                        logging.debug(
                            f"{question} was in the semantic memory!")
...
def main(memory_type: str, policy: dict, save_at: str, data_path: str,
         capacity: dict) -> None:
    ...
    if memory_type == 'episodic':
        ...
        train_only_semantic(policy, data, capacity, save_at)
    elif memory_type == 'both':
        train_both_episodic_and_semantic(
            policy, data, capacity, save_at)
    else:
        raise ValueError
...
if __name__ == "__main__":
    ...
    write_yaml(config, os.path.join(
        config['save_at'], "train-hand-crafted.yaml"))
    main(**config)
